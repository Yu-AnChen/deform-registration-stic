...
def load_roi_csv(path):
    ...
    condition = (
        roi["Name"].apply(lambda x: " | " not in x)
        & roi["Text"].apply(lambda x: " | " not in x)
        & roi["mpatch"].notnull()
        & roi["type"].apply(lambda x: "Poly" in x)
    )
    return roi.loc[condition].copy()
